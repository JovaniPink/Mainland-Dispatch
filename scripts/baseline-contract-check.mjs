...
if (excludedPaths.has(".next") || excludedPaths.has(".next/**")) {
  failures.push(
    "tsconfig.json must not exclude generated .next route validators"
  );
}
...
for (const metadataFile of requiredMetadataFiles) {
  try {
    ...
    failures.push(`server profile requires ${metadataFile}`);
  }
}
...
if (project.scripts?.prepare !== "node scripts/prepare.mjs") {
  failures.push("npm run prepare must execute: node scripts/prepare.mjs");
}

const prepareScenarios = [
  {
    label: "development dependencies omitted",
...
if (failures.length > 0) {
  console.error(`Server-profile contract failed:\n- ${failures.join("\n- ")}`);
  process.exitCode = 1;
} else {
  console.log(
    "Server-profile contract passed: metadata assets exist, production installs skip Git hooks, and generated route validators remain typechecked."
  );
}
