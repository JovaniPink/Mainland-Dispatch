...
const [project, nativeCompiler, compatibilityCompiler, renovate] =
  await Promise.all([
...
if (
  nativeCompiler.name !== "typescript" ||
  major(nativeCompiler.version) !== 7
) {
  failures.push(
...
const expectedScripts = {
  typecheck:
    "next typegen && node node_modules/@typescript/native/bin/tsc --noEmit",
  "typecheck:compat":
    "next typegen && node node_modules/typescript/bin/tsc --noEmit",
};
...
if (failures.length > 0) {
  console.error(
    `TypeScript toolchain contract failed:\n- ${failures.join("\n- ")}`
  );
  process.exitCode = 1;
} else {
  console.log(
